...
impl ConsoleOutput {
    ...
    pub fn display_summary(&self, report: &Report) -> Result<()> {
        ...
        if report.files.len() <= 20 {
            ...
            println!("\n{}", format!("({} files processed, use --sort to see details)", report.files.len()).yellow());
        }
    ...
    fn display_global_summary(&self, report: &Report) {
        ...
        table.add_row(Row::new(vec![
            ...
            Cell::new(&report.summary.logical_lines.to_formatted_string(&Locale::en)),
        ]));
        ...
        table.add_row(Row::new(vec![
            Cell::new("Languages"),
            Cell::new(&report.summary.languages_count.to_formatted_string(&Locale::en)),
        ]));
        
        // REQ-5.3: Calculate and display percentages with 2 decimal places
        if report.summary.total_lines > 0 {
            let logical_pct = (report.summary.logical_lines as f64 / report.summary.total_lines as f64) * 100.0;
            let empty_pct = (report.summary.empty_lines as f64 / report.summary.total_lines as f64) * 100.0;
    ...
    fn display_language_summary(&self, report: &Report) {
        ...
        match self.sort_metric {
            ...
            Some(SortMetric::Logical) => languages.sort_by_key(|l| std::cmp::Reverse(l.logical_lines)),
...
impl ReportExporter {
    ...
    fn export_xml(&self, report: &Report, path: &Path) -> Result<()> {
        let xml = serde_xml_rs::to_string(report)
            .map_err(|e| SlocError::Serialization(e.to_string()))?;
    ...
    fn export_csv(&self, report: &Report, path: &Path) -> Result<()> {
        let mut wtr = csv::Writer::from_path(path)
            .map_err(|e| SlocError::Io(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
        
        // Write header
        wtr.write_record(&["Path", "Language", "Total Lines", "Logical Lines", "Empty Lines"])
            .map_err(|e| SlocError::Io(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
        ...
        wtr.flush().map_err(|e| SlocError::Io(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
        Ok(())
    }
}